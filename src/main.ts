...
function configMatchesCurrentTable(key: string, data: any): boolean {
  ...
  const savedTableId = String(data?.tableId || '').trim();
  const savedAppToken = String(data?.appToken || '').trim();
  if (savedTableId && savedTableId !== state.tableId) return false;
  if (state.appToken && savedAppToken && savedAppToken !== state.appToken) return false;
  if (savedTableId === state.tableId) return true;
  if (state.appToken && key.endsWith(`:${state.appToken}:${state.tableId}`)) return true;
  return key.endsWith(`:${state.tableId}`);
}
...
function readSavedConfigText(): { key: string; value: string } | null {
  ...
  if (lastValue) {
    ...
    if (data && Array.isArray(data.flows) && configMatchesCurrentTable(LAST_CONFIG_STORAGE_KEY, data)) {
...
async function getAuthCode(): Promise<string> {
  ...
  const saved = readSavedConfigText();
  const savedData = saved ? parseSavedConfig(saved.value) : null;
  const savedToken = savedData?.authCode || savedData?.personalBaseToken;
  return typeof savedToken === 'string' && savedToken.trim() ? savedToken.trim() : '';
}
...
function applySavedConfigData(data: any): boolean {
  ...
  if (!state.authCode.trim()) {
    state.authCode = data.authCode || data.personalBaseToken || '';
  }
...
async function saveRemoteConfig() {
  ...
  const savingFlowId = state.activeFlowId;
  persistStateToActiveFlow();
  stopAutoSync();
  const { table } = await getTableAndView();
  const flow = activeFlow();
  if (flow.id !== savingFlowId) {
    throw new Error('当前流程在保存前发生变化，已停止保存以避免把 API key 写入其他流程。请重新选择流程后再保存。');
  }
      ...
      <section class="panel">
        ...
        <div class="grid">
          ...
          <div class="field">
            ...
            <input class="input" id="apiKey" data-flow-id="${escapeHtml(state.activeFlowId)}" type="password" value="${escapeHtml(state.apiKey)}" placeholder="请输入管理员提供的 API key" />
...
function syncFormValues() {
  ...
  const renderedFlowId = apiKeyInput?.dataset.flowId || '';
  if (renderedFlowId && renderedFlowId !== state.activeFlowId) {
    throw new Error('当前页面显示的流程与待保存流程不一致，已停止保存以避免 API key 写错流程。请重新选择流程后再试。');
  }
...
function bindForm() {
  ...
  document.querySelector<HTMLInputElement>('#apiKey')?.addEventListener('input', event => {
    const input = event.currentTarget as HTMLInputElement;
    if (input.dataset.flowId && input.dataset.flowId !== state.activeFlowId) return;
